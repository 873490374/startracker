...
In recent years there has been the development of microsatellites called CubeSats. CubeSat is a standard created in 1999 at the California Polytechnic State University\citet{heidt2000cubesat}, which is used for low-cost microsatellites. CubeSats are measured in units. Most are 1-U, 2-U and 3-U. CubeSat 1-U has a size of 10 cm on each edge and the maximum weight of 1.333 kg, while the CubeSat 2-U is 20x10x10 cm and can weigh up to 2.666 kg.
...
TODO
Zdjecie Cubesata
CubeSat missions, goals, examples, what can they be used for?
...
\subsubsection{Inertial Meaurement Unit}
...
\hline 
Star Sensors\newline (Scanners and Mappers) & Attitude accuraty = 1 arc sec to 1 arc min \newline 0.0003 deg to 0.01 deg \\ 
\hline 
Horizon Sensors\newline -Scanner/Pipper\newline -Fixed Head (Static) & Attitude accuracuracy: \newline 0.1 deg to 1 deg (LEO) \newline $<0.1$ deg to 0.25 deg \\ 
\hline 
...
magnitude threshold and for the star tracker maximum angular aperture. Essentially, the k-vector table is a structural database of all cataloged star pairs that could possibly fit in the camera FOV over the whole sky. The star pairs are ordered with increasing interstar angle.
The data stored are the k index, the cosine of the interstar angle, and the master catalog indices I[k] and J[k] of the kth star pair. The k-vector access logic is invoked in real time for a minimal set of star pairs in elementary measured star polygons (three for a triangle, six for a four-star pyramid, etc.); the fact that the vertices between adjacent measured star pairs share a common cataloged star is the key observation leading to logic for efficiently identifying the stars by simply comparing the k-vector accessed catalog indices from the several sets of candidate star pairs (which must contain the common measured pivot star, if
