...
\newglossaryentry{integer_vector}{type=symbols, name=$\bm{J}$, sort=integer_vector, description={Integer vector of the sorting}}
...
\newglossaryentry{unit_vector_inertial}{type=symbols, name=$\bm{r}$, sort=unit_vector_inertial, description={Known directional unit vector in the ECI frame}}
...
\end{equation}
\begin{equation}
\bm{bb}_2 = (\bm{bb}_1 x \bm{b}_2) /|\bm{bb}_1 x \bm{b}_2|
\end{equation}
\begin{equation}
\bm{bb}_3 = \bm{bb}_1 x \bm{bb}_2
\end{equation}
\end{subequations}
...
\begin{subequations}
\begin{equation}
\bm{rr}_1 = \bm{r}_1/|\bm{r}_1|
\end{equation}
\begin{equation}
\bm{rr}_2 = (\bm{rr}_1 x \bm{r}_2) /|\bm{rr}_1 x \bm{r}_2|
\end{equation}
\begin{equation}
\bm{rr}_3 = \bm{rr}_1 x \bm{rr}_2
\end{equation}
\end{subequations}

where $\bm{bb}_i, i\in\{1, 2, 3\}$ are matrices in body frame and $\bm{rr}_i, i\in\{1, 2, 3\}$ are matrices in reference frame.


Finally the attitude matrix $\bm{A}$ is computed:

\begin{equation}
\bm{A} = \bm{bb}_1 \cdot \bm{rr}_1^T + \bm{bb}_2 \cdot \bm{rr}_2^T + \bm{bb}_3 \cdot \bm{rr}_3^T
\end{equation}
...
Star Identifier algorithm was tested basing on \gls{esa}'s star-tracker simulator created for competition needs\cite{esa-competition}. This simulator creates sets of data: one that would be the actual output of star recognition algorithm, i.e. list of stars' centroids and magnitudes (x position, y position, magnitude); and the second set of data - list of stars' ids in Hipparcos catalog.
All tests were done with \gls{fov} 10\degree. On average algorithm using star triangle catalog of magnitude $\leqslant 5$ has accuracy above 90\%, visible in Figure \ref{fig:identification_accuracy}. While there is no satandardized deifinition of accuracy in this area, it is assumed that accuracy means the percent of scenes in tests where no star was recognized incorrectly. It means that algorithm could also not recognize any star from the input data in given scene.
Such particular accuracy definition comes from the fact that for given scene no star should be recognized incorrectly in order to get correct result in the next part of program, which is \gls{quest} algorithm. The percent of all identified stars in those test cases is around 95\% for stars within the catalog magnitude.

Test cases using star triangle catalog with magnitude $\leqslant 6$ proven to be also as accurate (\~95\%) for input data with stars with magnitude $\leqslant 5.56$, however the time of execution per scene rose significantly (from 50ms to 2.2s, 44 times). The advantage is that algorithm was able to identify significantly more stars - ~95\% versus ~35\% with cataloge magnitude 5. Test cases with input data with stars with magnitude $\leqslant 6$ ("esa" test scenarion) has worsen the accuracy - less scenes without incorrectly identified stars comparing to the same test with catalog magnitude $\leqslant 5$, while time rose significantly (from 200ms to 4.9s, 24.5 times). In order to make this algorithm more accurate on higher magnitude stars it is necessary to explore options of rewriting parts of this algorithm to GPU, using k-vector and finding better constant values for Planar Triangle algorithm.

\begin{figure}[!htbp]
\includegraphics[scale=0.55]{plot_star_identification_tests.png}
\centering
\caption[Star identification test results - accuracy and execution time]{Star identification test results - accuracy and execution time. Blue - percentage of scenes, where all identified stars are in the scene or were discarded, orange - percentage of scenes where all stars were correctly identified (star to star) or discarded, green - percent of all identified stars in all scenes and not discarded, red - average time of scene computation. Test scenarios' labels are with the following convention: mag4 means that stars in the input scenes are with magnitude up to 4, cat5 means that catalog used for testing has stars with magnitude up to 5; esa is special test scenario used in competition\cite{esa-competition} with 100 scenes, stars of magnitude up to 6 and multiple false stars. Other scenarions have no false stars and 1000 scenes per test}
\label{fig:identification_accuracy}
\end{figure}
...
\centering
\caption[Star-tracker execution time of each scene]{Star-tracker execution time of each scene and number of input stars.}
