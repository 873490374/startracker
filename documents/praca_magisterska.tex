...
Star-tracker program was written in Python 3.6 under Anaconda distribution run on ARM processor (on Jetson TX2) with usage of numba library for easy GPU computation. Python was chosen due to simplicity of programming - it is easier and faster to build a program. Additionally due to usage of libraries like numba and numpy it is only few times slower than if it would be written in C++. Of course it is possible to rewrite parts of program to C++ for better performance, however it will be around 5 times with a lot of additional work.
TODO write why Python is better!!!!!
numba is as fast as C++
