...
\item Star identification (which star in image corresponds to which star in on-board catalog) divided by design:
\begin{itemize}[noitemsep]
...
In 1965 Grace Wahba posed the problem of finding the proper orthogonal matrix $\bm{A}$ that minimizes the non-negative loss function\cite{wahba1965least}

\begin{equation}
L(\bm{A}) = \frac{1}{2} \sum_i^n|\bm{b}_i - \bm{Ar}_i|^2
\end{equation}
...
If the vectors are error-free and the true attitude matrix $\bm{A_{true}}$ is assumed to be the same for all the measurements, then $\bm{b}_i = \bm{A_{true}r}_i$ for each $i$ and the loss function $L(\bm{A}) = 0$ for $\bm{A} = \bm{A_{true}}$.
...
\begin{itemize}
\item Star recognition - recognizing stars on the image and converting the data into list of star vectors by calculating star centroids;
\item Star identification - identifying which star vector represents which real star in catalog. This is done by comparing star vectors from the image with data in star catalog, which is generated before space mission;
\item Attitude Estimation - estimating the attitude by calculating the displacement between two frames.
\end{itemize}

\subsection{Star recognition}
...
Please note that $a_{ROI}$ must be a positive odd number for the proper functioning of the algorithm and while higher value of pixel means higher light intensity, for star magnitude the lower value, the higher star brightness.
...
\item the sum of light intensity of given amount of pixels around the pixel is high enough (Equation \ref{eq:centroid_magnitude}), hence only low magnitude stars are taken into account. Describing the equation: it is the pixel intensity sum of vertical, horizontal and both diagonals of pixels around the given pixel, where $a_{MAG}$ is limit of pixels for magnitude ROI and $I_{thresh_{mag}}$ is limit threshold of star magnitude pixels intensity.
\begin{equation}
\begin{gathered}
...
{\includegraphics[width=0.5\textwidth]{centroids_4.png}\label{fig:centroids_4}}
  \caption[Example of centroids' clustering trade-off]{Example of centroids' clustering trade-off (in negative colors)}
  \label{fig:centroids-examples}
\end{figure}

\subsection{Star identification}

Star identification is the part of star-tracker program, which actually finds out which recognized stars are which. Prior this part it is only known that some stars were read from the image, and now it is the time to identify them by comparing to the on-board star catalog. There is a number of techniques to do that\cite{spratling2009survey}. In this section most of them will be described.
...
\begin{equation}
\rho = \cos^{-1}(\bm{r}_1 \cdot \bm{r}_2)
\end{equation}
where $\bm{r}_1$ and $\bm{r}_2$ are unit vectors pointing to each star. The vectors are given in inertial space. The angle $\rho$ is the same when inertial vectors or body vectors are used. The problem is that the angle measured between stars in the image contain significant measurement error, that cannot be ignored.
...
\bm{b}_i = A\bm{r}_i
\end{equation}
where $\bm{r}_i$ is direction the direction of star in the inertial coordinate system, $\bm{D}$ is the \gls{dcm}, $\bm{b}_i$ is direction of star in the star-tracker body coordinate system.

Nearly all the probability of errors is focused on a very small area about the direction of $\bm{D}\bm{r}_i$, that the sphere containing that point can be approximated by a tangent plane, characterized by:
\begin{equation}
\tilde{\bm{b}}_i = \bm{D}\bm{r}_i + \bm{v}_i, \hspace{0.5cm} \bm{v}_i^T\bm{D}\bm{r}_i = 0
\end{equation}
where $\tilde{\bm{b}}_i$ is the {i}th measurement and sensor error $\bm{v}_i$ is approximately Gaussian, which satisfies
\begin{subequations}
\begin{equation}
...
Instead of measuring the inter-star angle, this method creates spherical triangles out of three stars. Due to that more information can be obtained from such triangle than an angle, what enables to identify stars faster and using fewer stars overall than in previous method. In this technique the area and polar moment are used instead of angles. The main disadvantage comparing to angle matching method is that this one requires not two but three stars in order to work properly, however under typical conditions in angle matching it is necessary to use more stars due to measurement error. Also the spherical triangle catalog is about 10 times larger than angle catalog\cite{cole2004fast}.
...
\end{equation}
\end{subequations}
The partials with respect to $\bm{b}_1$, $\bm{b}_2$ and $\bm{b}_3$ are calculated with formulas:
\begin{subequations}
\begin{equation}
...
\begin{subequations}
\begin{equation}
\bm{u_a^T u_b} \geq \cos \gamma_{FOV}
\end{equation}
\begin{equation}
\bm{u_b^T u_c} \geq \cos \gamma_{FOV}
\end{equation}
\begin{equation}
\bm{u_c^T u_a} \geq \cos \gamma_{FOV}
\end{equation}
\label{eq:in_fov}
\end{subequations}
...
The k-vector technique works in the following way: $\bm{y}$ is the data vector of $n$ elements $(n \gg 1)$ and $\bm{s}$ is the same vector but sorted in ascending mode: $\bm{s}(i) \le \bm{s}(i+1)$, and $\bm{s}(i) = \bm{y}(\bm{J}(i))$, where $\bm{J}$ is the integer vector of the sorting with length $n$\cite{mortari2013k}.
...
or
\begin{equation}
\bm{k}(i) = j \hspace{0.5cm} \textnormal{where j is the greatest index such} \hspace{0.5cm} s(j) \leq \bm{y}	(\bm{J}(i)) \hspace{0.5cm} \textnormal{is satisfied.}
\end{equation}
...
Finally, having found $k_{start}$ and $k_{end}$ it is immediately known, that searched elements are $\bm{y}(i) \in [y_a, y_b]$, which are all the $\bm{y}(\bm{J}(i))$ where $k$ ranges from $k_{start}$ and $k_{end}$.
...
\gls{triad} is based on constructing two triads of orthonormal unit vectors using the vector information from input. Both triads are components of the same reference frame expressed in the body and inertial frames\cite{hall2003spacecraft}.

$\bm{b}_1$ and $\bm{b}_2$ are two vectors in body frame and $\bm{r}_1$ and $\bm{r}_2$ are in reference frame. First step of the algorithm is following:

\begin{subequations}
\begin{equation}
\bm{bb}_1 = \bm{b}_1/|\bm{b}_1|
\end{equation}
\begin{equation}
\bm{P}_2 = (\bm{P}_1 x \bm{b}_2) /|\bm{P}_1 x \bm{b}_2|
\end{equation}
\begin{equation}
\bm{P}_3 = \bm{P}_1 x \bm{P}_2
\end{equation}
\end{subequations}
...
\begin{subequations}
\begin{equation}
\bm{T}_1 = \bm{r}_1/|\bm{r}_1|
\end{equation}
\begin{equation}
\bm{T}_2 = (\bm{T}_1 x \bm{r}_2) /|\bm{T}_1 x \bm{r}_2|
\end{equation}
\begin{equation}
\bm{T}_3 = \bm{T}_1 x \bm{T}_2
\end{equation}
\end{subequations}

where $\bm{P}_i, i\in\{1, 2, 3\}$ are matrices in body frame and $\bm{T}_i, i\in\{1, 2, 3\}$ are matrices in reference frame.


Finally the attitude matrix $\bm{A}$ is computed:

\begin{equation}
\bm{A} = \bm{P}_1 \cdot \bm{T}_1^T + \bm{P}_2 \cdot \bm{T}_2^T + \bm{P}_3 \cdot \bm{T}_3^T
\end{equation}
...
Firstly the matrix $\bm{B}$ is calculated with $n$ observed stars, where $b_i$ is image star and $r_i$ catalog star for real star $i$:
\begin{equation}
...
\end{bmatrix}
\end{equation}
The \gls{dcm} $\bm{D}$ can be found by:
\begin{equation}
\bm{D = U_+V_+^T}
\end{equation}
...
\newpage
\section{Designed star-tracker program}

\subsection{Choice of tools and solutions}

Star-tracker program was written in Python 3.6 under Anaconda distribution run on ARM processor (on Jetson TX2) with usage of numba library\footnote{"Numba translates Python functions to optimized machine code at runtime using the industry-standard LLVM compiler library. Numba-compiled numerical algorithms in Python can approach the speeds of C or FORTRAN."\cite{numba}} for easy GPU computation. Python was chosen due to simplicity of programming - it is easier and faster to build a program. Additionally due to usage of libraries like numba and numpy\footnote{"NumPy is the fundamental package for scientific computing with Python. It contains among other things: a powerful N-dimensional array object, sophisticated (broadcasting) functions, tools for integrating C/C++ and Fortran code, useful linear algebra, Fourier transform, and random number capabilities."\cite{numpy}} it is only few times slower than if it would be written in C++\cite{numba1}\cite{numba2}\cite{numpy1}. Of course it is possible to rewrite parts of program to C++ for better performance, however it will be around 2-10 times faster with a lot of additional work.

Using CUDA on GPU was chosen because it made program run significantly faster (around 31.6 times faster). The exact difference is visible in Figure \ref{fig:cuda_vs_no_cuda_centroid}.

\begin{figure}[!htbp]
\includegraphics[scale=0.55]{cuda_vs_no_cuda_centroid_plot.png}
\centering
\caption{Star recognition (centroid calculation): GPU vs CPU}
\label{fig:cuda_vs_no_cuda_centroid}
\end{figure}

\subsection{Comparison of designed star-tracker with existing solutions}
TODO ADD STARTRACKER COMPARISON TABLE!!!

\subsection{Star-tracker design}
...
\begin{itemize}[noitemsep]
\item image star - star in the image;
\item image star ID - ID of star recognized in the image (serves as ordinal number);
\item catalog star - star in the Hipparcos catalog;
\item Hipparcos star ID - ID of the star in Hipparcos catalog;
\item false star - object initially recognized as star that is later identified as false star (marked with -1), it can be planet, satellite, star with magnitude higher than in catalog, etc.
\end{itemize}
...
\subsubsection{Star recognition}
...
Centroid algorithm was tested using images created with Stellarium, free planetarium software, which made possible to mock input to star-tracker camera. Images are with following parameters: \gls{fov} 10\degree and 900x900 pixels, and imitate parts of that should be visible by star-tracker on the actual orbit.
...
\item $a_{MAG}$ is star size limit in pixels,
\item $a_{ROI}$ is size of ROI in pixels.
\end{itemize}
...
Star Identifier algorithm was tested basing on \gls{esa}'s star-tracker simulator created for competition needs\cite{esa-competition}. This simulator creates data two sets of data: one that would be the actual output of star recognition algorithm, i.e. list of stars' centroids and magnitudes (x position, y position, magnitude); and the second set of data - list of stars' ids in Hipparcos catalog.
...
Tracking works in the following way: if there are no previously found stars - work with the input frame as in LIS mode, then go to tracking. If there are already previously found stars, then list of  planar triangles from those stars is created, which is used later instead of on-board catalog for searching common triangles. The rest works similarly as in LIS mode: common triangles are found and the best options are chosen. After that the attitude is estimated.

Computational complexity of identifying stars by tracking is $O(n^3+n^2)$, where $n$ is number of star vectors.

Tracking is around twice faster than LIS mode run on the same data, what is visible in Figure \ref{fig:tracking_vs_lis}. The disadvantage of tracking mode is that it has to have at least 3 stars from the last LIS found stars in the new image. The higher speed comes from the fact that in the tracking mode program has significantly lower number of triangles to search in.

\begin{figure}[!htbp]
\includegraphics[scale=0.55]{tracking_vs_lis_time_plot.png}
\centering
\caption{Average time for star identification: Tracking vs LIS modes}
\label{fig:tracking_vs_lis}
\end{figure}
...
Next, for each combination of 3 image stars, eligible star are calculated and added list of eligible stars for each image star. Table \ref{tab:example_2} shows lists of eligible stars after doing it for all combinations. Only 10 most common occurrences of Hipparcos star ID are shown in the table due to long list. It is clearly visible, that stars 1 and 2 have significantly smaller number of highest occurring Hipparcos star IDs.
...
After that each image star is being assigned its Hipparcos star ID according to algorithm described in previous chapter.
...
\item rewrite CUDA parts of program to pycuda or to C/C++ because the Python Anaconda distribution for ARM architecture is experimental and it could simply break on satellite,
...
\item rewrite tracking mode to work beyond stars found by LIS mode,
\item optimize Planar Triangle algorithm,
\item use catalog with more stars,
