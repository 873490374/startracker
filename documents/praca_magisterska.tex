...
Another platform for slightly larger than CubeSat satellites was created by Polish company Creotech. This platform is called HyperSat and is based on experiences of CubeSat: it is also modular and universal. In the smallest configuration satellite can have dimensions of 30x30x10 cm and weight od 10 kg, while at largest it can have 30x30x60 cm and weight 60 kg. Because of the larger size and weight HyperSat can have more applications than CubeSat.
...
In order to research possibility of decreasing costs of designing and producing new hardware for startracker, part of the thesis is to run the startracker program on off-the-shelf device: NVIDIA Jetson TX2. 
Price of this device is around 570 USD (around 2200 PLN).
...
Centroid algorithm was tested using images created with Stellarium, free planetarium software, which made possible to mock input into the startracker. Input images are with following parameters: FOV 10\degree and 900x900 pixels, and imitate parts of that should be visible by startracker on the actual orbit.

\subsubsection{Star identifier}

This means that algorithm rejects stars that can be false stars (other satellite, planet, etc.) or star with higher magnitude than in the catalog.
...
First way of testing was giving on input image of sky and then go through all parts of the program. The images were created using Stellarium program, which imitates part of sky with given FOV and coordinates. The parameters of images used during testing are:
\begin{itemize}[noitemsep]
\item FOV 10\degree,
\item 900x900 pixels.
\end{itemize}
...
Second way of testing was showing image of sky on the separate screen and try to read it with camera on JETSON device. Although program clearly was able to find stars in the camera input, it needs precise configuration in order to find centroids accurately. Surely such testing will have to be done in case of using this startracker with dedicated camera.
...
- rewrite program (or at least GPU parts) to C/C++ as the Python Anaconda distribution for ARM architecture is experimental and it could break on the satellite.
...
- explore other algorithms, especially for star identification and attitude estimation, because it is possible new research improved existing algorithms or created new ones with significantly higher efficiency.
